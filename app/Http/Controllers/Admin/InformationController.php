...
class InformationController extends Controller
{
    ...
    public function topHeaderOffer(){
		$results=Information::where('id','13')->orwhere('id','14')->orwhere('id','15')->get();
		return view('admin.information.headeroffer',compact('results'));
	}
			...
			if ($validator->fails()){
				...
				return response(array('message'=>$message),403);
					...
					try{
						...
						foreach($request->post('id') as $key=>$id){

							if($id>0){
							
								$Information=Information::find($id);
								$Information->description=$request->post('description')[$key];
