		...
		if($request->isMethod('post')){
			...
			if ($validator->fails()){
				...
				if(!$parent && $request->post('parent_id')!=''){
					...
					$category->business_type=$request->post('business_type');
		...
		if($result){
			...
			if(!empty($childId)){
				...
				foreach($childId as $child){
					...
					Category::where('id',$child)->update(['recyclebin_status'=>'1','recyclebin_datetime'=>date('Y-m-d H:i:s')]);
				}
			}

			Category::where('id',$id)->update(['recyclebin_status'=>'1','recyclebin_datetime'=>date('Y-m-d H:i:s')]);
