			...
			<td style="padding: 0 30px 0;">
				<table style="text-align:left;background: #fff;width: 100%;border:1px solid #111;border-top:0;border-collapse: collapse;">
					...
					<tr class="table-heading">
						...
						@if($result[0]['country_id']=='101')
							<th style="border: 1px solid #111;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">CGST</p></th>
							<th style="border: 1px solid #111;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">SGST</p></th>
							<th style="border: 1px solid #111;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">IGST</p></th>
						@endif						
					...
					@foreach($result as $key=>$value)
						...
						<?php
							...
							$variantResult=\App\Models\Variantproduct::where('id',$value['product_id'])->first(); 
							...
							if($value['country_id']=='101'){
								...
								$unitPrice=round((($value['amount']*100)/(100+$taxPercentage)),2);
							}

							$totalUnitPrice+=$unitPrice;

							$cgsgAmount=0;$igstAmount=0;

							if($value['state_id']=='4016'){

								$cgsgAmount=$value['amount']-$unitPrice;

								$totalCgsgAmount+=$cgsgAmount;
				
							}else{
				
								$igstAmount=$value['amount']-$unitPrice;

								$totalIgstAmount+=$igstAmount;

							}
						
							$amount=round(($unitPrice+$cgsgAmount+$igstAmount),2);
						...
						<tr> 
							...
							<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($unitPrice,2) }}</p></td>
							@if($result[0]['country_id']=='101')
								<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($cgsgAmount,2) }}</p></td>
								<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($cgsgAmount,2) }}</p></td>
								<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($igstAmount,2) }}</p></td>
							@endif					
							<td style="border: 1px solid #111;border-top:0;border-bottom:0;padding: 0 8px;text-align:right"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($amount,2) }}</p></td>	
						</tr>
					@endforeach
					...
					<tr style="border-bottom:1px solid black"> 
						...
						<td style="border: 1px solid #111;border-bottom:0;padding: 0 8px;text-align:left"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($totalCgsgAmount,2) }}</p></td> 
						<td style="border: 1px solid #111;border-bottom:0;padding: 0 8px;text-align:left"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($totalCgsgAmount,2) }}</p></td> 
						<td style="border: 1px solid #111;border-bottom:0;padding: 0 8px;text-align:right"><p style="font-size: 12px;margin: 5px 0;">{{ number_format($totalIgstAmount,2) }}</p></td> 
